...
def DFormer_Small(pretrained=False, **kwargs):
    ...
    if pretrained:
        # 直接传入权重路径，绕过类型检查
        weights_path = "/home/panyi/Desktop/ACANet/backbone/DFormer/DFormer_Small.pth.tar"
...
def load_model_weights(model, model_type, kwargs):
    ...
    Args:
        model: 模型实例
        model_type: 模型类型，如 "scnet"
        kwargs: 额外参数，可能包含路径信息
    ...
    # 根据模型类型选择权重文件路径
    if model_type == "scnet":
        if "DFormer_Small" in str(model.__class__):
            weights_path = "/home/panyi/Desktop/ACANet/backbone/DFormer/DFormer_Small.pth.tar"
        elif "DFormer_Base" in str(model.__class__):
            weights_path = "/home/panyi/Desktop/ACANet/backbone/DFormer/DFormer_Base.pth.tar"
        else:
            raise ValueError(f"不支持的模型类型: {model.__class__}")
    else:
        raise ValueError(f"不支持的权重类型: {model_type}")
